...
class User(db.Model):
    ...
    def set_password(self, raw_password: str):
        ...
        # stash old hash in history if we have one
        if getattr(self, "hash", None):
            ph = PasswordHistory(salt=self.salt, hash=self.hash)
            ph.user = self  # ① link the FK
            db.session.add(ph)  # ② put it in the session

            # keep only last N
            q = (PasswordHistory.query
                 .filter_by(user_id=self.id)
                 .order_by(PasswordHistory.created.desc()))
            for old in q.offset(Config.PASS_HISTORY_COUNT):
