...
def register():
    ...
    Duplicate e-mails are ignored; all other validation removed.
    """
    if request.method == "POST":
        ...
        # any single quote will break the statement → perfect for the demo
        raw_sql = (
        ...
        try:
            db.session.execute(text(raw_sql))     # SQL-i gateway
            db.session.commit()
            flash("VULN register: row inserted (SQL-i possible).", "warning")
        except Exception as e:                    # duplicate e-mail or SQL error
            db.session.rollback()
...
def login():
    """
    Raw SELECT with user input → ' OR 1=1 --  bypass works.
    """
    if request.method == "POST":
        username = request.form["username"]
        password = request.form["password"]      # ignored in query

        q = f"SELECT id FROM user WHERE username = '{username}' AND hash = '{password}'"
        row = db.session.execute(text(q)).fetchone()
        if row:
            session["uid"] = row[0]
            flash("Logged-in via VULN endpoint!", "warning")
            return redirect(url_for("dashboard"))

        flash("Bad creds (vuln route).", "danger")
